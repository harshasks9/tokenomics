...
## 11. v6 (July 2026): build-vs-buy reassessment of the account universe

**Principle change.** Accounts are no longer selected for size, digital sophistication, or interaction volume alone. The final test per account: *"a material business problem AND a credible reason not to build the full conversational-AI platform internally."* The universe is now dominated by buy-led and partner-led targets.

**Classification framework** (`BuildVsBuy` on every account): buy-led (limited internal ability/appetite — packaged solution) · partner-led (capable, needs workflows/integration/managed ops) · co-build (strong engineering — sell models/infra/governance) · build-led (deprioritized; pursue only for a specific gap).

**Universe churn (~19 swaps).** Deprioritized with evidence (public internal-AI platforms, launched assistants, build-first engineering cultures): Flipkart (Flippi/GenAI org), Meesho (in-house GenAI voice support bot), Swiggy, Zomato, MakeMyTrip (Myra), Paytm, PhonePe, CRED, Tokopedia (ByteDance stack), Grab, Shopee/Sea, Lazada (Alibaba), Rakuten (own LLM), Mercari, PayPay, LY Corp, Coupang, KakaoBank, Baemin. Added buy-led replacements: LIC, Mahindra Finance, Bandhan Bank, Indian Hotels (Taj), TVS Motor, Max Healthcare, Blue Dart, Amity University, Astra International, Vietnam Airlines, Minor International, Astro Malaysia, ANA, AEON, Dai-ichi Life, Korean Air, Hanwha Life, AGL Energy, Medibank. Exclusion log rendered on the dashboard ("Selection logic") with rationale, evidence, and pursue-only-if conditions per company.

**New per-account evidence fields** (all required): `buildVsBuy` + `buildVsBuyWhy` (classification evidence) · `whyNotBuild` (right to win) · `existing.has / existing.gaps` (what exists — don't pitch it — vs what customers still can't complete end-to-end) · `earningsSignals` (1–3 per account: signal, source style "Q2 FY26 earnings call"/"FY25 annual report", approximate date, concrete GTM implication, kind = fact/inference/hypothesis; fabricated quotes/figures/URLs prohibited — uncertain items downgraded to inference with "validate" flags).
