...
Live route: `/brief` (also served at `brief.aitokenomics.app` once the domain is attached to the Vercel project).
...
- Vercel cron hits `GET /api/brief/cron` daily at **13:00 UTC** (see `vercel.json`) with `Authorization: Bearer $CRON_SECRET`.
