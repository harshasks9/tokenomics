...
function exportCsv(levers: Levers, result: ModelResult) {
  const rows: Array<Array<string | number>> = [
    ...
    ["LEVERS"],
    ["Lever", "Value"],
    ["New capacity (peak-sized GSUs)", levers.gsus],
    ["Utilization if peak-sized", levers.uPeak],
    ["Right-sized PT utilization", levers.uPt],
    ["Workload on PT (raw)", levers.mix.pt],
    ["Spike share (raw)", levers.mix.spike],
    ["Off-peak share (raw)", levers.mix.offPeak],
    ["Deferred share (raw)", levers.mix.deferred],
    ["Batch share (raw)", levers.mix.batch],
    ["Workload on PT (normalized)", result.mix.pt],
    ["Spike share (normalized)", result.mix.spike],
    ["Off-peak share (normalized)", result.mix.offPeak],
    ["Deferred share (normalized)", result.mix.deferred],
    ["Batch share (normalized)", result.mix.batch],
    ...
    ["  eligible (200+ committed GSUs)", result.bogoEligible ? "yes" : "no"],
    ...
    [],
    ["INCREMENTAL COMMIT"],
    ["Committed GSUs / month", result.gsu.units.toFixed(1)],
    ...
    [],
    ["SUMMARY"],
    ["At list $K/mo", result.atList.toFixed(2)],
    ["Placed & committed $K/mo", result.final.toFixed(2)],
    ["Monthly saving $K", (result.atList - result.final).toFixed(2)],
    ...
    ["Peak-sized effective multiplier", result.peakMultiplier.toFixed(4)],
    [],
    ["SAVING ATTRIBUTION ($K per month)"],
    ["Right-sizing", result.attribution.rightSizing.toFixed(2)],
