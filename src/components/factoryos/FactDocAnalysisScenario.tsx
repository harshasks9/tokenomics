...
import { ScanLine, Zap, CheckCircle2 } from "lucide-react";
import { fmtUSD, pctSavings } from "@/lib/pricing";
import { FACT_S3, FACT_S3_CARDS, factS3Costs, type FactoryQualityCard } from "@/lib/industries/factoryos";
...
const LANE_CONFIG: Record<LaneKey, { label: string; color: string; badge?: string }> = {
  ...
  hybrid:     { label: "Hybrid — Flash scan → Opus disposition",   color: INDIGO, badge: "Best Value" },
...
export default function FactDocAnalysisScenario() {
  const { updateResult } = useTally();
  const [selectedCard, setSelectedCard] = useState<FactoryQualityCard>(FACT_S3_CARDS[0]);
  const [running, setRunning]           = useState(false);
  const [done, setDone]                 = useState<Set<LaneKey>>(new Set());
  const [inspectionsPerYear, setInspectionsPerYear] = useState(FACT_S3.defaults.inspectionsPerYear);

  const costs = factS3Costs();
  const annualPureClaude = costs.pureClaude * inspectionsPerYear;
  const annualHybrid     = costs.hybrid     * inspectionsPerYear;
  const annualPureGemini = costs.pureGemini * inspectionsPerYear;
  ...
  const handleRace = () => {
    ...
    lanes.forEach((key) => {
      ...
      }, FACT_S3.lanes[key].latencyMs);
    });
    const maxMs = Math.max(...Object.values(FACT_S3.lanes).map((l) => l.latencyMs));
    setTimeout(() => setRunning(false), maxMs);
  };

  const fmtMs = (ms: number) => ms >= 1000 ? `${(ms / 1000).toFixed(1)}s` : `${ms}ms`;
  const fmtCount = (n: number) => n >= 1_000_000 ? `${(n / 1_000_000).toFixed(1)}M` : `${(n / 1_000).toFixed(0)}K`;
  const orderedLanes: LaneKey[] = ["pureClaude", "hybrid", "pureGemini"];

  return (
    <section id="fact-quality" className="relative py-24 bg-white">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          transition={{ duration: 0.6 }} className="text-center mb-14">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium mb-6"
            style={{ background: AMBER + "12", color: AMBER }}>
            <ScanLine size={16} /> Scenario 3 — Run Phase
          </div>
        ...
        <div className="grid lg:grid-cols-2 gap-10">
          {/* LEFT: part selector + findings */}
          <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}
            transition={{ duration: 0.5 }} className="space-y-4">
            <p className="text-xs font-medium uppercase tracking-wider text-gray-400">Select a part type</p>
            <div className="space-y-2">
              {FACT_S3_CARDS.map((card) => (
                <button key={card.id} onClick={() => setSelectedCard(card)}
                  className={`w-full text-left rounded-xl border-2 p-4 transition-all ${
                    selectedCard.id === card.id ? "shadow-md" : "border-gray-200 hover:border-gray-300 bg-white"
                  }`}
                  style={selectedCard.id === card.id ? { borderColor: AMBER, background: AMBER + "06" } : {}}>
                  <div className="flex items-start gap-3">
                    <ScanLine size={16} className="mt-0.5 shrink-0" style={{ color: selectedCard.id === card.id ? AMBER : "#9CA3AF" }} />
                    <div>
            ...
            <div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
              <div className="px-4 py-2.5 border-b border-gray-100">
                <span className="text-xs font-semibold text-gray-700">Inspection Findings — {selectedCard.title}</span>
              </div>
              <table className="w-full text-xs">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="text-left px-4 py-2 font-medium text-gray-400 uppercase tracking-wide text-[10px]">Check</th>
                    <th className="text-left px-4 py-2 font-medium text-gray-400 uppercase tracking-wide text-[10px]">Result</th>
                    <th className="text-left px-4 py-2 font-medium text-gray-400 uppercase tracking-wide text-[10px]">Signal</th>
                  </tr>
                </thead>
                <tbody>
                  {selectedCard.findings.map((f, i) => (
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
              <div className="flex items-center justify-between mb-5">
                <p className="text-sm font-bold text-gray-800">Inspection Pipeline Race</p>
                <button onClick={handleRace} disabled={running}
                  ...
                  style={{ background: running ? "#9CA3AF" : AMBER }}>
                  <Zap size={12} /> {running ? "Scanning…" : "Run Inspection"}
                </button>
              </div>
              <div className="space-y-5">
                {orderedLanes.map((key) => {
                  const lane = FACT_S3.lanes[key];
                  ...
                  return (
                    <div key={key}>
                      ...
                      <ProgressBar latencyMs={lane.latencyMs} color={cfg.color} isRunning={running} done={isDone} />
                      <div className="flex justify-between mt-1 text-[10px] text-gray-400">
                        <span>Quality: {"★".repeat(lane.quality)}{"☆".repeat(5 - lane.quality)}</span>
                        <span>{fmtUSD(costs[key], 4)}/part</span>
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                <p className="text-xs font-medium uppercase tracking-wider text-gray-400">Inspections / Year</p>
                <span className="text-lg font-bold tabular-nums" style={{ color: AMBER }}>{fmtCount(inspectionsPerYear)}</span>
              </div>
              <input type="range" min={500_000} max={20_000_000} step={100_000} value={inspectionsPerYear}
                onChange={(e) => setInspectionsPerYear(Number(e.target.value))}
                className="w-full h-2 cursor-pointer accent-orange-600 mb-4" />
            ...
            <div className="rounded-2xl p-5" style={{ background: `linear-gradient(135deg, ${AMBER} 0%, #7c2d12 100%)` }}>
              <p className="text-xs font-semibold uppercase tracking-wider text-orange-200 mb-1">Hybrid vs Pure Opus</p>
              <p className="text-3xl font-extrabold text-white tabular-nums mb-1">
                {fmtUSD(annualPureClaude - annualHybrid)} saved/yr
              </p>
              <p className="text-orange-200 text-sm">
                {pctSavings(annualPureClaude, annualHybrid).toFixed(0)}% reduction at {fmtCount(inspectionsPerYear)} parts/year ·
                same Opus disposition quality
              </p>
            </div>
          </motion.div>
        </div>
