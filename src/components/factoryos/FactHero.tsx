...
const phases = [
  { label: "Build", sub: "IoT Platform SDLC" },
  { label: "Run",   sub: "Floor Assistant" },
  { label: "Run",   sub: "Quality Inspection" },
  { label: "Extend", sub: "Predictive Maintenance" },
];
...
export default function FactHero() {
  return (
    <section
      ...
      style={{ background: "linear-gradient(160deg, #0c0a06 0%, #1c1407 40%, #0c0a06 70%, #1a1206 100%)" }}
    >
      {/* Grain overlay */}
      <div
      ...
      />
      {/* Glow */}
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[700px] h-[300px] rounded-full blur-[140px] opacity-10"
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6 lg:px-8 py-20">
        ...
        >
          64% build savings.
          <br />
          <span style={{ color: AMBER }}>Zero compromise on the floor.</span>
        </motion.h1>
        ...
        >
          Factory codebases are 55% boilerplate. Tiered routing routes it to Flash-Lite.
          Safety interlocks and SCADA integration stay on Opus.
          Right model, right task — across the full production lifecycle.
        </motion.p>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }}
          ...
          {phases.map((p, i) => (
            <span key={i} className="flex items-center rounded-full border border-white/10 bg-white/5 px-4 py-1.5 text-xs font-semibold text-white/70">
              {p.label}
              <span className="ml-1.5 text-xs text-white/60">{p.sub}</span>
            </span>
          ))}
        </motion.div>
