...
import { FACT_S4, FACT_S4_TRACE, factS4Costs, factS4Annual, type FactS4Config, type FactAgentNode } from "@/lib/industries/factoryos";
...
function nodeColor(node: FactAgentNode, cfg: FactS4Config): string {
  if (node.role === "planner" || node.role === "reviewer") return AMBER;
  return cfg === "allOpus" ? AMBER : BLUE;
}

function roleIcon(role: FactAgentNode["role"]) {
  if (role === "planner")  return <Bot className="h-4 w-4" />;
  if (role === "reviewer") return <ShieldCheck className="h-4 w-4" />;
  return <Cpu className="h-4 w-4" />;
}

function NodePopover({ node, cfg, onClose }: { node: FactAgentNode; cfg: FactS4Config; onClose: () => void }) {
  ...
  return (
    <motion.div
      ...
      className="absolute z-50 bottom-full left-1/2 -translate-x-1/2 mb-2 w-60 rounded-lg border border-gray-200 bg-white shadow-xl p-4">
...
function TraceNode({ node, index, cfg, isVisible, selectedId, onSelect }: {
  node: FactAgentNode; index: number; cfg: FactS4Config; isVisible: boolean;
  ...
  return (
    <motion.div className="relative"
      ...
      transition={{ duration: 0.4, delay: isVisible ? index * 0.07 : 0, ease: "easeOut" }}>
      <button onClick={() => onSelect(isSelected ? null : node.id)}
        className="group relative flex items-center gap-2 rounded-lg px-3 py-2 text-left transition-all cursor-pointer"
        style={{
...
export default function FactAgentScenario() {
  const { updateResult } = useTally();
  const sectionRef = useRef<HTMLDivElement>(null);
  const isInView   = useInView(sectionRef, { once: true, margin: "-100px" });

  const [cfg, setCfg]             = useState<FactS4Config>("tiered");
  const [machines, setMachines]   = useState(FACT_S4.defaults.machinesPerRun);
  const [selectedId, setSelectedId] = useState<number | null>(null);

  const costsAllOpus  = factS4Costs("allOpus");
  const costsTiered   = factS4Costs("tiered");
  const annualAllOpus = factS4Annual(machines, "allOpus");
  const annualTiered  = factS4Annual(machines, "tiered");
  ...
  const fmtMachines = (v: number) => v >= 1_000 ? `${(v / 1_000).toFixed(0)}K` : v.toString();

  const planner  = FACT_S4_TRACE.filter((n) => n.role === "planner");
  const executors = FACT_S4_TRACE.filter((n) => n.role === "executor");
  const reviewer = FACT_S4_TRACE.filter((n) => n.role === "reviewer");
  const chosen   = cfg === "allOpus" ? costsAllOpus : costsTiered;

  return (
    <section id="fact-agent" ref={sectionRef} className="relative w-full bg-gray-50/50 py-24">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }}
          className="mb-14">
          <div className="flex items-center gap-2 mb-4">
            <span className="rounded-full px-3 py-1 text-xs font-semibold tracking-wide uppercase" style={{ background: AMBER + "15", color: AMBER }}>Scenario 4</span>
          ...
          <h2 className="text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">Nightly Predictive Maintenance Agent</h2>
          <p className="mt-4 max-w-2xl text-lg leading-relaxed text-slate-600">
            Reviews 3,000 monitored machines per night — pulling vibration FFTs, thermal snapshots, and
            lubrication logs. 13 data-fetch steps on Flash; priority triage and supervisor dispatch on Opus.
          </p>
        </motion.div>
        ...
        <div className="grid gap-12 lg:grid-cols-[1fr_400px]">
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.5, delay: 0.1 }}
            ...
            <div className="flex gap-2">
              {(["tiered", "allOpus"] as FactS4Config[]).map((c) => (
                <button key={c} onClick={() => setCfg(c)}
                  className="rounded-lg px-4 py-1.5 text-xs font-semibold transition-all border"
                  style={{
                    backgroundColor: cfg === c ? (c === "tiered" ? AMBER : AMBER) + "15" : "transparent",
                    borderColor: cfg === c ? AMBER : "#e2e8f0",
                    color: cfg === c ? AMBER : "#64748b",
                  }}>
            ...
            <div className="space-y-4">
              ...
              <div>
                <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-2">
                  Executors ({FACT_S4.steps}) — {cfg === "allOpus" ? "Opus" : "Gemini Flash"}
                </p>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 gap-2">
                  {reviewer.map((node, i) => (
                    <TraceNode key={node.id} node={node} index={FACT_S4.steps + 2} cfg={cfg} isVisible={isInView} selectedId={selectedId} onSelect={handleSelect} />
                  ))}
                </div>
              </div>
            </div>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.5, delay: 0.2 }}
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
              <p className="text-xs font-medium uppercase tracking-wider text-gray-400 mb-4">Cost per machine run</p>
              {[
                { label: "Planner (Opus)",   val: chosen.plannerCost,  color: AMBER },
                { label: `Executors ×${FACT_S4.steps} (${cfg === "allOpus" ? "Opus" : "Flash"})`, val: chosen.executorCost, color: cfg === "allOpus" ? AMBER : BLUE },
                ...
                <div key={r.label} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                  ...
                  <span className="text-xs font-bold tabular-nums" style={{ color: r.color }}>{fmtUSD(r.val, 4)}</span>
                </div>
              ))}
              <div className="flex items-center justify-between pt-3 mt-1">
                <span className="text-sm font-bold text-gray-900">Total / machine</span>
                <span className="text-sm font-extrabold tabular-nums" style={{ color: cfg === "allOpus" ? AMBER : INDIGO }}>
                  {fmtUSD(chosen.total, 4)}
                </span>
              </div>
              <div className="mt-3 pt-3 border-t border-gray-100 text-xs text-gray-400">
                Savings vs all-Opus: {fmtUSD(costsAllOpus.total - costsTiered.total, 4)}/machine ({pctSavings(costsAllOpus.total, costsTiered.total).toFixed(0)}%)
              </div>
            </div>

            <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                <p className="text-xs font-medium uppercase tracking-wider text-gray-400">Machines / Night</p>
                <span className="text-lg font-bold tabular-nums" style={{ color: AMBER }}>{fmtMachines(machines)}</span>
              </div>
              <input type="range" min={100} max={20_000} step={100} value={machines}
                onChange={(e) => setMachines(Number(e.target.value))}
                className="w-full h-2 cursor-pointer accent-orange-600 mb-2" />
              <div className="flex justify-between text-[10px] text-gray-400"><span>100</span><span>20K</span></div>
            </div>

            <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm space-y-3">
              <p className="text-xs font-medium uppercase tracking-wider text-gray-400">Annual Cost (365 nights)</p>
              {[
            ...
            <div className="rounded-2xl p-5" style={{ background: `linear-gradient(135deg, ${AMBER} 0%, #7c2d12 100%)` }}>
              <p className="text-xs font-semibold uppercase tracking-wider text-orange-200 mb-1">Annual Savings</p>
              ...
              <p className="text-orange-200 text-sm">
                ...
                Opus on 2 of {FACT_S4.steps + 2} steps only
              </p>
            </div>
          </motion.div>
        </div>
