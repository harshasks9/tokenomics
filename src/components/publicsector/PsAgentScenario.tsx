...
function nodeColor(node: PsAgentNode, cfg: PsS4Config): string {
  if (node.role === "planner" || node.role === "reviewer") return AMBER;
  return cfg === "allOpus" ? AMBER : BLUE;
}

function roleIcon(role: PsAgentNode["role"]) {
    ...
    >
      ...
      <div className="space-y-1.5 text-xs">
        <div className="flex justify-between">
          ...
          <span className="font-semibold" style={{ color }}>{modelLabel}</span>
        </div>
        <div className="flex justify-between">
...
function TraceNode({ node, index, cfg, isVisible, selectedId, onSelect }: {
  node: PsAgentNode; index: number; cfg: PsS4Config; isVisible: boolean;
...
export default function PsAgentScenario() {
  const { updateResult } = useTally();
  const sectionRef = useRef<HTMLDivElement>(null);
  const isInView = useInView(sectionRef, { once: true, margin: "-100px" });

  const [cfg, setCfg] = useState<PsS4Config>("tiered");
  const [contracts, setContracts] = useState(PS_S4.defaults.contractsPerRun);
  const [selectedId, setSelectedId] = useState<number | null>(null);

  const costsAllOpus = psS4Costs("allOpus");
  const costsTiered  = psS4Costs("tiered");
  const annualAllOpus = psS4Annual(contracts, "allOpus");
  const annualTiered  = psS4Annual(contracts, "tiered");

  useEffect(() => {
    updateResult("s4", {
      label: "Procurement Compliance Agent",
  ...
  const fmtContracts = (v: number) => v >= 1_000 ? `${(v / 1_000).toFixed(0)}K` : v.toString();

  const planner  = PS_S4_TRACE.filter((n) => n.role === "planner");
  const executors = PS_S4_TRACE.filter((n) => n.role === "executor");
  const reviewer = PS_S4_TRACE.filter((n) => n.role === "reviewer");

  const chosen = cfg === "allOpus" ? costsAllOpus : costsTiered;

  return (
    <section id="ps-agent" ref={sectionRef} className="relative w-full bg-gray-50/50 py-24">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }}
          className="mb-14">
          <div className="flex items-center gap-2 mb-4">
            <span className="rounded-full px-3 py-1 text-xs font-semibold tracking-wide uppercase" style={{ background: INDIGO + "15", color: INDIGO }}>Scenario 4</span>
          ...
          <h2 className="text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">Nightly Procurement Compliance Agent</h2>
          <p className="mt-4 max-w-2xl text-lg leading-relaxed text-slate-600">
            Reviews 5,000 active contracts per night — checking CPARS ratings, milestone dates, small business targets,
            and invoice approval status. 14 data-pull steps on Flash; compliance sign-off on Opus.
          </p>
        </motion.div>
        ...
        <div className="grid gap-12 lg:grid-cols-[1fr_400px]">
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.5, delay: 0.1 }}
            ...
            <div className="flex gap-2">
              {(["tiered", "allOpus"] as PsS4Config[]).map((c) => (
                <button key={c} onClick={() => setCfg(c)}
                  className="rounded-lg px-4 py-1.5 text-xs font-semibold transition-all border"
                  style={{
                    backgroundColor: cfg === c ? (c === "tiered" ? INDIGO : AMBER) + "15" : "transparent",
                    borderColor: cfg === c ? (c === "tiered" ? INDIGO : AMBER) : "#e2e8f0",
                    color: cfg === c ? (c === "tiered" ? INDIGO : AMBER) : "#64748b",
                  }}>
            ...
            <div className="space-y-4">
              ...
              <div>
                <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-2">
                  Executors ({PS_S4.steps}) — {cfg === "allOpus" ? "Opus" : "Gemini Flash"}
                </p>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 gap-2">
                  {reviewer.map((node, i) => (
                    <TraceNode key={node.id} node={node} index={PS_S4.steps + 2} cfg={cfg} isVisible={isInView} selectedId={selectedId} onSelect={handleSelect} />
                  ))}
                </div>
              </div>
            </div>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.5, delay: 0.2 }}
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
              <p className="text-xs font-medium uppercase tracking-wider text-gray-400 mb-4">Cost per contract run</p>
              {[
                { label: "Planner (Opus)",  val: chosen.plannerCost,  color: AMBER },
                { label: `Executors ×${PS_S4.steps} (${cfg === "allOpus" ? "Opus" : "Flash"})`, val: chosen.executorCost, color: cfg === "allOpus" ? AMBER : BLUE },
              ...
              <div className="flex items-center justify-between pt-3 mt-1">
                <span className="text-sm font-bold text-gray-900">Total / contract</span>
              ...
              <div className="mt-3 pt-3 border-t border-gray-100 text-xs text-gray-400">
                Savings vs all-Opus: {fmtUSD(costsAllOpus.total - costsTiered.total, 4)}/contract ({pctSavings(costsAllOpus.total, costsTiered.total).toFixed(0)}%)
              </div>
            </div>

            {/* Contract scale slider */}
            <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                <p className="text-xs font-medium uppercase tracking-wider text-gray-400">Contracts / Night</p>
                <span className="text-lg font-bold tabular-nums" style={{ color: INDIGO }}>{fmtContracts(contracts)}</span>
              </div>
              <input type="range" min={500} max={50_000} step={500} value={contracts}
                onChange={(e) => setContracts(Number(e.target.value))}
                className="w-full h-2 cursor-pointer accent-indigo-600 mb-2" />
              <div className="flex justify-between text-[10px] text-gray-400"><span>500</span><span>50K</span></div>
            </div>
            ...
            <div className="rounded-2xl p-5" style={{ background: `linear-gradient(135deg, ${INDIGO} 0%, #312E81 100%)` }}>
              <p className="text-xs font-semibold uppercase tracking-wider text-indigo-200 mb-1">Annual Savings</p>
              ...
              <p className="text-indigo-300 text-sm">
                ...
                Opus on 2 of {PS_S4.steps + 2} steps only
              </p>
            </div>
          </motion.div>
        </div>
