...
export default function DealPanel({ meta, inputs, result, onMeta, onLoad }: {
  ...
  const exportFile = () => {
    const blob = new Blob([serializeDeal(toDealFile(meta, inputs, result))], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a"); a.href = url; a.download = dealFileName(meta); a.click();
    setTimeout(() => URL.revokeObjectURL(url), 1000);
  };
  const importFile = async (f: File | undefined) => {
    if (!f) return;
    try { const d = parseDeal(await f.text()); onLoad(d); flash(`Loaded "${d.meta.customer || f.name}".`); }
    catch (e) { flash(`Could not load: ${e instanceof Error ? e.message : "unknown error"}`); }
  ...
  const loadSaved = (key: string) => {
    ...
    if (d) { onLoad(d); flash(`Loaded "${d.meta.customer}".`); }
  };
  return (
    <div className="dc-card">
      <div className="dc-field" style={{ marginTop: 0 }}>
      ...
      <label className="dc-check"><input type="checkbox" checked={meta.onTargetList} onChange={(e) => set({ onTargetList: e.target.checked })} /><span>On the 40-account target list</span></label>
      <div className="dc-toolbar" style={{ marginTop: 10, marginBottom: 0 }}>
        <button className="dc-btn" onClick={exportFile}>Export deal file</button>
        <button className="dc-btn" onClick={() => fileRef.current?.click()}>Import</button>
      ...
      <details className="dc-group">
        <summary>Save, revisit, notes</summary>
        ...
        <div className="dc-toolbar" style={{ marginTop: 10 }}>
          <button className="dc-btn primary" onClick={save}>Save in this browser</button>
          {saved.some((d) => d.meta.customer === meta.customer) && <button className="dc-btn" onClick={() => { deleteDeal(meta.customer); flash(`Removed "${meta.customer}".`); }}>Delete</button>}
        </div>
      ...
      {msg && <p className="dc-presetnote">{msg}</p>}
    </div>
  );
}
