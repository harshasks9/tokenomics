    ...
    <dl>
      {terms.map((t) => (
        <div key={t.label} style={{ display: "contents" }}>
          ...
          <dd>{typeof t.value === "object" ? JSON.stringify(t.value) : String(t.value)}{t.note ? ` — ${t.note}` : ""}</dd>
        </div>
      ))}
    </dl>
  );
...
export function Methodology() {
  return (
    <details>
      ...
      <p><b>Google Private Offer.</b> Requires a new GCP commitment of at least $10M iACV. For the twelve months from signing, each quarter&apos;s marketplace spend above the last-full-quarter baseline earns the credit rate, capped cumulatively at the account cap. Credits settle at quarter end and can only be applied against eligible Cloud AI spend from the following month.</p>
