...
import { HC_SELLER_NOTES } from "@/lib/industries/healthcare";
...
export default function HcSummaryDashboard() {
  ...
  const handleCopy = () => {
    const lines = [
      "Healthcare AI Tokenomics Summary",
      "=================================",
      "",
      `S1 Build: All-Frontier ${fmtT(buildAllFrontier)} vs Tiered ${fmtT(buildTiered)} → ${fmtT(buildAllFrontier - buildTiered)} saved (one-time)`,
      `S2 Clinical Assistant: All-Frontier ${fmtT(s2?.allFrontier ?? 0)}/yr vs Tiered ${fmtT(s2?.tiered ?? 0)}/yr → ${fmtT(s2?.savings ?? 0)}/yr`,
      `S3 Document Analysis: All-Frontier ${fmtT(s3?.allFrontier ?? 0)}/yr vs Tiered ${fmtT(s3?.tiered ?? 0)}/yr → ${fmtT(s3?.savings ?? 0)}/yr`,
      `S4 Care Agent: All-Frontier ${fmtT(s4?.allFrontier ?? 0)}/yr vs Tiered ${fmtT(s4?.tiered ?? 0)}/yr → ${fmtT(s4?.savings ?? 0)}/yr`,
      "",
  ...
  const sellerNotes = [HC_SELLER_NOTES.s1, HC_SELLER_NOTES.s2, HC_SELLER_NOTES.s3, HC_SELLER_NOTES.s4];

  return (
    <section id="hc-summary" className="bg-[#F8F9FA] border-t border-[#E8EAED]">
      <div className="section-container">
          ...
          {[
            ...
            <div key={row.label} className="card">
              ...
              <div className="flex items-center justify-between">
                ...
                {row.all > 0 && (
                  <span className="text-xs font-semibold text-[#34A853] bg-emerald-50 rounded-full px-2 py-0.5">
        ...
        <div className="grid lg:grid-cols-2 gap-8 items-start">
          <div className="card">
            ...
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={chartData} margin={{ top: 0, right: 0, bottom: 0, left: 0 }} barGap={4}>
                ...
                <Bar dataKey="All-Frontier" fill="#E37400" radius={[4, 4, 0, 0]} maxBarSize={36} />
          ...
          <div className="space-y-4">
            {/* Annual savings hero */}
            <div className="rounded-2xl p-6" style={{ background: "linear-gradient(135deg, #188038 0%, #14662d 100%)" }}>
              <p className="text-xs font-semibold uppercase tracking-wider text-emerald-200 mb-2">Total Annual Savings</p>
            ...
            <div className="card space-y-3">
              ...
              {[
                ...
                { key: "s2", label: "S2 Clinical Asst.",  result: s2 },
                { key: "s3", label: "S3 Doc Analysis",    result: s3 },
                { key: "s4", label: "S4 Care Agent",      result: s4 },
