...
export function buildDecisionBrief(economics: ApproachEconomics[], profile: EstateProfile): DecisionBrief {
  ...
  return {
    ...
    headline: isGeap
      ...
      : `${APPROACH_META[winner.approach].shortName} is the strongest fit for this estate`,
    rationale: isGeap
      ...
      : "This estate is contained enough that an assembled approach avoids platform overhead while keeping full architectural flexibility. The assembly burden is manageable at this scope.",
    economicDriver: isGeap
      ...
      : "At lower scale, existing-tool reuse and narrower capability requirements keep assembly effort below the integrated platform commitment.",
    architectureDriver: hasCustomer
      ...
      : "The estate can be served with a narrower production footprint, keeping architecture straightforward.",
    tradeoff: APPROACH_META[winner.approach].tradeoff,
    flipConditions: [
      "High existing-tool coverage significantly reduces the assembly burden of an alternative approach.",
      "A meaningfully lower engineering cost base changes the build-vs-buy calculus on integration work.",
      isGeap
        ? "A very contained estate with simple, bounded agents may not need the full managed stack."
        : "Scaling to customer-facing or orchestrating agents raises governance requirements substantially.",
    ],
  };
}
